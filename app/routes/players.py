...
async def search_players(request: Request, query: str):
    ...
    await rate_limiter.check_rate_limit(
        key=f"search_players:{client_ip}", 
        limit=5, 
        window=60 
    )
...
async def get_player_profile(request: Request, player_id: str):
    ...
    await rate_limiter.check_rate_limit(
        key=f"player_profile:{client_ip}", 
        limit=5, 
        window=60 
    )
...
async def get_player_transfers(
    ...
    await rate_limiter.check_rate_limit(
        key=f"player_transfers:{client_ip}", 
        limit=5, 
        window=60 
    )
...
async def get_player_injuries(
    ...
    await rate_limiter.check_rate_limit(
        key=f"player_injuries:{client_ip}", 
        limit=5, 
        window=60 
    )
...
async def get_player_absences(
    ...
    await rate_limiter.check_rate_limit(
        key=f"player_absences:{client_ip}", 
        limit=5, 
        window=60 
    )
